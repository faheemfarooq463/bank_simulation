...
def update_time():   #this function will run and will fetch date and time of system,,, #An extra code which is to be written after curdate of line 18
    curdate=time.strftime("%d-%b-%Y 🕒%r")
    date.configure(text=curdate)
    date.after(1000,update_time)     #shows time after each one second
...
def forgot_screen():
    ...
    def send_otp():    
        ...
        query='''select name,email,pass from accounts where acn=? and adhhar=?'''
...
def welcome_screen(acn=None):
    ...
    def check_screen():
        ...
        ifrm.place(relx=.2 ,rely=.1,relwidth=.7,relheight=.65)
        ...
        conobj=sqlite3.connect(database='mybank.sqlite')
        curobj=conobj.cursor()
        query='''select acn,bal,adhhar,email,opendate from accounts where acn=?'''
...
'''
    ...
    def update_screen():
        ...
        conobj=sqlite3.connect(database='mybank.sqlite')
        curobj=conobj.cursor()
        query='''select name,email,pass,mob,pass from accounts where acn=?'''
        curobj.execute(query,(acn,))
        tup=curobj.fetchone()
        conobj.close() 
        ...
        lbl_pass=Label(ifrm,text='Pass',width=7,font=('arial',20,'bold'),bg='purple',fg='white')
        ...
        submit_btn=Button(ifrm,text='Submit',bg='powder blue',
    ...
    lbl_welcome=Label(frm,text=f'Welcome,{tup[0]}',font=('arial',20,'bold'),bg='purple',fg='white')
    lbl_welcome.place(relx=.001,rely=0)
  
    def update_pic():
        name=filedialog.askopenfilename()
        os.rename(name,f"{acn}.jpg")
        img_profile=Image.open(f'{acn}.jpg').resize((270,220))
        imgtk2_profile=ImageTk.PhotoImage(img_profile,master=root)  
        lbl_img_profile=Label(frm,image=imgtk2_profile) 
        lbl_img_profile.place(relx=.001,rely=.05)
        lbl_img_profile.image=imgtk2_profile

    if os.path.exists(f'{acn}.jpg'):
        img_profile=Image.open(f'{acn}.jpg').resize((270,220))
    else:
        img_profile=Image.open('default.jpg').resize((270,220))
    ...
    deposit_btn=Button(frm,text='Deposit Amount',width=15,bg='green',font=('arial',20,'bold'),bd=5,command=deposit_screen)
    deposit_btn.place(relx=.001,rely=.7) 

    withdraw_btn=Button(frm,text='Withdraw Amount',width=15,bg='red',font=('arial',20,'bold'),bd=5,command=withdraw_screen)
    withdraw_btn.place(relx=.001,rely=.8) 

    transfer_btn=Button(frm,text='Transfer Amount',width=15,bg='red',font=('arial',20,'bold'),bd=5,command=transfer_screen)
...
def newuser_screen():
    ...
    submit_btn=Button(frm,text='Submit',bg='powder blue',font=('arial',20,'bold'),bd=5,command=createacn_db)
    submit_btn.place(relx=.35,rely=.5)

    reset_btn=Button(frm,text='Reset',bg='powder blue',font=('arial',20,'bold'),bd=5,)
    reset_btn.place(relx=.5,rely=.5)

def main_screen():           #to create a frame 

    def newuser_click():
         frm.destroy()        #nested function
         newuser_screen()
    ...
    frm=Frame(root,highlightbackground='black',highlightthickness=2)      #putting frame in root window 
    frm.configure(bg='pink')
    frm.place(relx=0,rely=.16  ,relwidth=1,relheight=.75)       #relwidth of frame is that of root window,s,relheight=.8 
...
root=Tk()                #this code makes top level window
root.state('zoomed')     #to make fullscreen window
root.resizable(width=False,height=False)    #resizing disable so that its doesnt resize again
root.configure(bg='powder blue')        

title=Label(root,text="Banking Simulation",
            font=('arial',50,'bold','underline'),bg='powder blue')   #here we make label to show title,,,can be seen in root window     
title.pack()         #takes a compound to the top center of the window 

curdate=time.strftime("%d-%b-%Y 🕒%r")
date=Label(root,text=curdate,
            font=('arial',20,'bold'),bg='powder blue',fg='blue')
date.pack(pady=15)      #date.pack will be below title.pack which is Banking Stimulation,,,pady=15 will create a gap between title and date
update_time() 

img=Image.open('logo.jpg').resize((200,140))
imgtk=ImageTk.PhotoImage(img,master=root)  

lbl_img=Label(root,image=imgtk)
lbl_img.place(relx=0,rely=0)

img2=Image.open('logo2.jpg').resize((200,140))
imgtk2=ImageTk.PhotoImage(img2,master=root)  

lbl_img2=Label(root,image=imgtk)
lbl_img2.place(relx=.86,rely=0)

footer=Label(root,text="Developed By:Faheem \n 📞6006017475",
